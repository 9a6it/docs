...
В большинстве проектов Vue с поддержкой инструментов сборки мы создаем компоненты Vue, используя формат файла, подобный HTML, называемый Single-File Component (также известный как *.vue файлы, сокращенно SFC). Vue SFC, как следует из названия, инкапсулирует логику компонента (JS), шаблон (HTML) и стили (CSS) в одном файле. Вот предыдущий пример, написанный в формате SFC:
...
Эта команда установит и выполнит `create-vue`, официальный инструмент для развертывания проектов на Vue. Вам будут показаны диалоги для нескольких дополнительных опций, таких как TS и поддержка тестирования.
...
- Если вы решите использовать TS, ознакомьтесь с руководством по использованию TS.
...
Чтобы запустить локальный HTTP-сервер, сначала установите Node.js а затем запустите команду `npm install -g http-server` для установки простейшего сервера глобально. Затем из командной строки в том же каталоге, где находится ваш HTML-файл, запустите команду `http-server` для запуска сервера.
...
<div>{{ if (isOk) { return message } }}</div>
```
{% endraw %}
...
Директивы - это специальные атрибуты с префиксом `v-`. Vue предоставляет ряд встроенных директив, включая `v-html` и `v-bind`, которые мы описали выше.

Ожидается, что значения атрибутов директивы будут отдельными выражениями JS (за исключением `v-for`, `v-on` и `v-slot`, которые будут рассмотрены в соответствующих разделах позже). Задача директивы заключается в реактивном применении обновлений к DOM при изменении значения ее выражения. Возьмем в качестве примера `v-if`:

```
<div v-if="seen">...</div>
```

Здесь директива `v-if` удалит / вставит элемент `<div>...</div>` на основе истинности значения переданного выражения, коим выступает в данном случае свойство компонента `seen`.

#### Аргументы директив

Некоторые директивы могут принимать "аргумент", обозначаемый двоеточием после имени директивы. Например, директива `v-bind` используется для реактивного обновления атрибута HTML и принимает имя атрибута в качестве аргумента:
...
Здесь `href` является аргументом, который указывает директиве `v-bind` привязать атрибут `href` элемента к значению `url`. В сокращенном варианте все, что находится перед аргументом (т.е. `v-bind:`), сводится к одному символу `:`.
...
Здесь аргументом является название события для прослушивания: `click`. `v-on` имеет соответствующее сокращение, а именно символ `@`. Мы также поговорим об обработке событий более подробно позже.

#### Динамические аргументы

Также возможно использовать выражение JS в аргументе директивы, заключив его в квадратные скобки:
...
Здесь имя атрибута (`attributeName`) будет динамически вычисляться как выражение JS, и его вычисленное значение будет использоваться в качестве конечного значения для аргумента. Например, если у вашего экземпляра компонента есть свойство данных `attributeName`, значение которого равно "href", то эта привязка будет эквивалентна `v-bind:href`.

Аналогично, вы можете использовать динамические аргументы для привязки обработчика к имени динамического события:
...
В этом примере, если у экземпляра компонента есть свойство данных `eventName` равное `focus`, шаблон вычисляется как `v-on:focus`.
